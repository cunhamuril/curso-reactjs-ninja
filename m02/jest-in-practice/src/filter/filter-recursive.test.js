...
it('filter([5, 2, 3, 5], item => item === 5) should return [5, 5]', () => {
  expect(filter([5, 2, 3, 5], item => item === 5)).to.be.deep.equal([5, 5])
})
...
// Remover itens duplicado do array
it('filter([1, 2, 3, 2, 1, 5], (item, index, array) => index === array.indexOf(item)) ' +
  'should return [1, 2, 3, 5]', () => {
    expect(
      filter([1, 2, 3, 2, 1, 5], (item, index, array) => index === array.indexOf(item))
